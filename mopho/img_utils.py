...
def generate_album_thumbnails(photos_basedir, thumbs_basedir, album_name, single_photo_name=None):
    ...
    for fn in [p for p in pics if os.path.isfile(p) and not p.startswith(".")]:
        results.append(pool.apply_async(generate_thumbnails, (photos_basedir, thumbs_basedir, album_name, fn)))
...
def generate_thumbnails(photos_basedir, thumbs_basedir, album_name, filename):
    """
    ...
    :return:
    """
    full_fn = "%s/%s" % (photos_basedir, get_photo_relpath(album_name, filename))

    for res in RESOLUTIONS:
        out_fn = "%s/%s" % (thumbs_basedir, get_thumb_relpath(album_name, filename, res[0]))
...
def get_thumb_relpath(album_name, photo_name, width):
    """
    ...
    return "%s/%s-width-%d.jpg" % (album_name, photo_name, width)
